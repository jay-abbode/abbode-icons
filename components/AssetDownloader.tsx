...
export default function AssetDownloader({
  ...
  const scopedIcons = useMemo(() => {
    // Status gate: active always, drafts only when opted in, archived never.
    let list = icons.filter((i) => {
      const s = i.status.toUpperCase();
      if (s === "ARCHIVED") return false;
      if (s === "DRAFT") return includeDrafts;
      return true;
    });
    if (onlyNew) list = list.filter((i) => newSet.has(i.slug));
    return list;
  }, [icons, onlyNew, newSet, includeDrafts]);
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <section className="rounded-2xl border border-parchment bg-white p-5">
        ...
        <label className="font-ui mt-3 flex cursor-pointer items-start gap-2 border-t border-parchment pt-3 text-sm text-espresso">
          <input
            type="checkbox"
