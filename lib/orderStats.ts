...
// ---------------------------------------------------------------------------
// Popularity sort helpers (browse page "Most popular" toggle).
// Built on getOrderStats() above so we reuse its 60s cache and ORDER_STATS
// reader rather than hitting the sheet a second time.
// ---------------------------------------------------------------------------

/**
 * Normalizes an icon name for joining the catalog (MASTER) to ORDER_STATS:
 * lowercased, with runs of whitespace collapsed. Both sides come from the same
 * source names, so this is just defensive against stray spacing/case.
 */
...
/**
 * Map of normalized icon name -> order count over the rolling window, derived
 * from getOrderStats(). Used by the browse page to sort by popularity.
 * Returns {} on any error so the sort degrades to alphabetical.
 */
export async function getIconOrderCounts(): Promise<Record<string, number>> {
  try {
    const snap = await getOrderStats();
    const counts: Record<string, number> = {};
    for (const s of snap.stats) {
      const key = normIconName(s.icon);
      if (!key) continue;
      // Keep the largest if a name appears more than once.
      counts[key] = Math.max(counts[key] ?? 0, s.count);
    }
    return counts;
  } catch {
    return {};
